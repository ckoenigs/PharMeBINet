...
def create_cypher_file(file_name, node_label, rela_name, to_CT):
    ...
    if to_CT:
        ...
        query = ''' MATCH (d:CellType{identifier:line.CT_id}),(c:%s{identifier:line.other_id}) CREATE (d)<-[: %s{resource: ['Cell Ontology'], co: True, source:"Cell Ontology", licenses:["%s"], url:"https://www.ebi.ac.uk/ols4/ontologies/cl/classes?obo_id="+line.CT_id}]-(c)'''
    else:
...
def create_tsv_file_and_cypher_query(label, rela_type, to_CT):
    ...
    csv_writer.writerow(['CT_id','other_id'])
    create_cypher_file(file_name, label, rela_type, to_CT)
    
    return csv_writer


def load_all_pair_and_add_to_files(label):
    """
...
def main():
    ...
    for label in ['Anatomy','BiologicalProcess','CellularComponent','MolecularFunction']:
        load_all_pair_and_add_to_files(label)
    cypher_file.close()
    driver.close()
