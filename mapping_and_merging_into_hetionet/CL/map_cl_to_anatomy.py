...
def generate_tsv_files_and_cypher_file():
    ...
    query_match = pharmebinetutils.get_query_import(path_of_directory,
                                                    f'mapping_and_merging_into_hetionet/CL/{file_name_mapped}',
                                                    query_match)
...
def map_hetionet_anatomy():
    ...
    counter = 0
    counter_mapped = 0
    query = f'Match (n:{label_co}) Where split(n.id,":")[0]="UBERON" Return n.id'
    results = g.run(query)
