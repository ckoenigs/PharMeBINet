...
def generate_files(path_of_directory):
    ...
    header = ['ptmd_identifier', 'identifier', 'resource', 'mapping_method', 'licenses']
    ...
    query = f' Match (n:PTMD_Protein{{uniprot_accession:line.ptmd_identifier}}), (v:Protein{{identifier:line.identifier}}) Set v.ptmd=True, v.resource=split(line.resource,"|"), v.licenses=split(line.licenses,"|") Create (v)-[:equal_to_PTMD_protein{{mapped_with:line.mapping_method}}]->(n)'
    ...
    for record in results:
        ...
        if identifier in dict_identifier_to_resource:
            ...
            csv_mapping.writerow([
                ...
                pharmebinetutils.resource_add_and_prepare(dict_identifier_to_resource[identifier][0], "PTMD"),
                'uniprot_accession', '|'.join(dict_identifier_to_resource[identifier][1])
            ])
            counter_mapped_id += 1
        ...
        for main_id, alternatives in dict_identifier_to_alternative_ids.items():
            if identifier in alternatives:
                dict_identifier_to_resource[main_id][1].add(license)
                csv_mapping.writerow([
                    identifier, main_id,
                    pharmebinetutils.resource_add_and_prepare(dict_identifier_to_resource[main_id][0], "PTMD"),
                    'alternative_id', '|'.join(dict_identifier_to_resource[main_id][1])
                ])
