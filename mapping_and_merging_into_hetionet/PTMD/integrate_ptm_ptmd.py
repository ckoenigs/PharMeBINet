...
def generate_files(path_of_directory):
    ...
    query = (f' Match (n:PTMD_PTM) WHERE id(n) = toInteger(line.nodeId) MERGE (v:PTM{{identifier:line.identifier}})  '
             f'On Create Set  v.ptmd=True, v.source="PTMD", v.resource=split(line.resource,"|"), v.url="https://ptmd.biocuckoo.cn/index.php",'
             f' v.residue=line.residue, v.licenses=["%s"], '
             f'v.position=line.position, v.type=line.type Create (v)-[:equal_to_PTMD_ptm]->(n)')
    query = query % (pharmebinetutils.dict_source_to_license['ptmd'])
...
def load_all_PTMD_ptms_and_finish_the_files(csv_mapping):
    ...
    query = ("MATCH (n:PTMD_PTM)-[:PTMD_HAS_PTM]-(v:PTMD_Protein)--(p:Protein) RETURN DISTINCT id(n) AS nodeId, n.position, n.residue, "
    ...
    for nodeId, position, residue, ptm_type, uniprot_accession, identifier in results:
        if position is not None and residue is not None:
            ...
            if unique_identifier not in set_identifier:
                csv_mapping.writerow(
                    [nodeId, unique_identifier, "PTMD", position, residue, ptm_type])
                counter_all += 1
                set_identifier.add(unique_identifier)
        else:
            print(nodeId, position, residue, ptm_type)
            counter_not_mapped += 1

    print('number of new pmts:', counter_all)
