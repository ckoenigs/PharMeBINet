...
def load_all_pair_and_add_to_files(label, equal_type, rela_types):
    """
    ...
    for rela_type in rela_types:
        for [direction_one, direction_two, to_cT] in [['', '>', False], ['<', '', True]]:
            ...
            query = f'''MATCH (p:Food)-[:equal_to_FO]-(r){direction_one}-[v:{rela_type}]-{direction_two}(n)-[:{equal_type}]-(b:{label}) RETURN p.identifier, b.identifier, type(v)'''
...
def main():
    ...
    list_label_edge_type = [['Anatomy', 'equal_anatomy_fo', ['is_a']], ['Chemical', 'equal_foodon_chemical',
                                                                        ['composed_primarily_of',
                                                                         'has_defining_ingredient', 'has_ingredient',
                                                                         'has_part','is_a']]]

    for [label, equal_type, rela_types] in list_label_edge_type:
        load_all_pair_and_add_to_files(label, equal_type, rela_types)
    cypher_file.close()
    driver.close()
