...
def generate_tsv_files_and_cypher_file():
    ...
    query_match = query_start + ' , (l:Anatomy{identifier:line.id}) Set l.foodon=true, l.resource=split(line.resource,"|"), l.licenses=split(line.licenses,"|") Create (l)-[:equal_anatomy_fo]->(a)'

    cypher_file = open('output/cypher.cypher', 'a', encoding='utf-8')

    query_match = pharmebinetutils.get_query_import(path_of_directory,
                                                    f'mapping_and_merging_into_hetionet/foodon/{file_name_mapped}',
                                                    query_match)
...
def map_foodon_anatomy():
    ...
    counter = 0
    counter_mapped = 0
    query = f'Match (n:{label_fo}) Where split(n.id,":")[0]="UBERON" Return n.id'
    results = g.run(query)
    for record in results:
        ...
        if node_id in dict_anatomy_id_to_resource:
            ...
            dict_anatomy_id_to_resource[node_id][1].add(pharmebinetutils.dict_source_to_license['foodon'])
            csv_update.writerow(
                [node_id,
                 pharmebinetutils.resource_add_and_prepare(resource, 'FoodOn'),
...
def main():
    ...
    print('Map anatomy nodes of foodon')
