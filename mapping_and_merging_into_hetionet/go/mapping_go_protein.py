...
def generate_files(label, cypher_file):
    ...
    query = '''Match (n:protein_go{identifier:line.identifier}), (v:%s{identifier:line.other_id}) Set v.go=True, v.resource=split(line.resource,"|"), v.licenses=split(line.licenses,"|") Create (v)-[:equal_to_go_%s{how_mapped:line.mapped_with}]->(n)'''
...
def load_protein_and_add_to_dictionary():
    ...
    for record in results:
        ...
        dict_uniprot_id_to_resource[identifier] = [node['resource'], set(node['licenses'])]
...
def write_to_tsv_file(tsv_writer,identifier, other_identifier, mapping_method, dict_id_to_resource):
    dict_id_to_resource[other_identifier][1].add(pharmebinetutils.dict_source_to_license['go'])
    tsv_writer.writerow(
        [identifier, other_identifier,
         pharmebinetutils.resource_add_and_prepare(dict_id_to_resource[other_identifier][0], 'GO'),
         mapping_method, '|'.join(dict_id_to_resource[other_identifier][1])])
...
def load_all_protein_form_go_and_map_and_write_to_file():
    ...
    for record in results:
        ...
        if identifier in dict_uniprot_id_to_resource:
            write_to_tsv_file(tsv_writer, identifier, identifier, 'id_mapped', dict_uniprot_id_to_resource)
            ...
            for real_id in dict_alternative_uniprot_id_to_identifiers[identifier]:
                write_to_tsv_file(tsv_writer, identifier, real_id, 'alt_id_mapped', dict_uniprot_id_to_resource)
        else:
        ...
        if symbol in dict_unique_gene_symbol_to_gene_ids:
            ...
            for gene_id in dict_unique_gene_symbol_to_gene_ids[symbol]:
                write_to_tsv_file(tsv_writer_gene, identifier, gene_id, 'unique_symbol', dict_gene_id_to_resource)
        elif symbol in dict_symbol_to_gene_ids:
            count_mapped_symbol += 1
            for gene_id in dict_symbol_to_gene_ids[symbol]:
                write_to_tsv_file(tsv_writer_gene, identifier, gene_id, 'symbol', dict_gene_id_to_resource)
        elif symbol in dict_synonyms_to_gene_ids:
            count_mapped_symbol += 1
            for gene_id in dict_synonyms_to_gene_ids[symbol]:
                write_to_tsv_file(tsv_writer_gene, identifier, gene_id, 'symbol_with_synonyms', dict_gene_id_to_resource)
        else:
