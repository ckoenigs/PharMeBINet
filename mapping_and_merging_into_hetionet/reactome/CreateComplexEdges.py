...
def create_cypher_file(file_path, node_label, rela_name, direction1, direction2):
    if node_label == "Protein" or node_label == "Chemical":
        query = '''Using Periodic Commit 10000 LOAD CSV  WITH HEADERS FROM "file:%smapping_and_merging_into_hetionet/reactome/%s" As line FIELDTERMINATOR "\\t" MATCH (d:MolecularComplex{identifier:line.id_hetionet_Complex}),(c:%s{identifier:line.id_hetionet_node}) CREATE (d)%s[:%s{order:line.order, stoichiometry:line.stoichiometry, source:'Reactome', url:"https://reactome.org/content/detail/"+line.id_hetionet_Complex, compartments: split(line.compartment, "|"), resource: ['Reactome'], reactome: "yes", license:"CC BY 4.0"}]%s(c);\n'''
    else:
        query = '''Using Periodic Commit 10000 LOAD CSV  WITH HEADERS FROM "file:%smapping_and_merging_into_hetionet/reactome/%s" As line FIELDTERMINATOR "\\t" MATCH (d:MolecularComplex{identifier:line.id_hetionet_Complex}),(c:%s{identifier:line.id_hetionet_node}) CREATE (d)%s[:%s{order:line.order, stoichiometry:line.stoichiometry, resource: ['Reactome'], reactome: "yes", source:'Reactome', url:"https://reactome.org/content/detail/"+line.id_hetionet_Complex, license:"CC BY 4.0"}]%s(c);\n'''
...
def check_relationships_and_generate_file(new_relationship, node_reactome_label,  node_hetionet_label,
    ...
    if "Chemical" in node_hetionet_label or "Protein" in node_hetionet_label:
        csv_mapped.writerow(['id_hetionet_Complex', 'id_hetionet_node', 'order', 'stoichiometry', 'compartment'])
    else:
