...
def create_cypher_file(file_path, node_label, rela_name, direction1, direction2):
    if node_label == "Protein" or node_label == "Chemical":
        query = ''' MATCH (d:MolecularComplex{identifier:line.id_pharmebinet_Complex}),(c:%s{identifier:line.id_pharmebinet_node}) CREATE (d)%s[:%s{order:line.order, stoichiometry:line.stoichiometry, source:'Reactome', url:"https://reactome.org/content/detail/"+line.id_pharmebinet_Complex, compartments: split(line.compartment, "|"), resource: ['Reactome'], reactome: true, licenses:["%s"]}]%s(c)'''
    else:
        query = ''' MATCH (d:MolecularComplex{identifier:line.id_pharmebinet_Complex}),(c:%s{identifier:line.id_pharmebinet_node}) CREATE (d)%s[:%s{order:line.order, stoichiometry:line.stoichiometry, resource: ['Reactome'], reactome: true, source:'Reactome', url:"https://reactome.org/content/detail/"+line.id_pharmebinet_Complex, licenses:["%s"]}]%s(c)'''
