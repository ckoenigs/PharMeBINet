...
def load_iuphar_ids_in():
    with open('C:/Users/Dietrich/Downloads/neo4j-community-3.5.4/ligands.csv', newline='', encoding="utf-8") as csvfile:
...
def load_hetionet_drug_in():
    query = '''MATCH (n:Chemical) WHERE not n:Product RETURN n.identifier, n.name, n.inchi, n.resource;'''
...
def load_reactome_drug_in():
    ...
    for drug_node, in results:
        ...
        drug_name = drug_node['inn'].lower() if "inn" in drug_node else drug_node['inn']            #INN - international nonproprietary name
        drug_names = drug_node['name'] if "name" in drug_node else []

        if iuphar_reactome in dict_iuphar_to_inchi_hetionet:
            counter_map_with_id += 1
            hetionet_identifier = dict_hetionet_inchi_drugbank_identifier[dict_iuphar_to_inchi_hetionet[iuphar_reactome]]
...
def create_cypher_file():
    cypher_file = open('drug/cypher.cypher','w', encoding="utf-8")
    query = '''Using Periodic Commit 10000 LOAD CSV  WITH HEADERS FROM "file:///C:/Users/Dietrich/PycharmProjects/reactome_projekt/drug/mapped_drug.tsv" As line FIELDTERMINATOR "\\t" MATCH (d:Compound{identifier:line.id_hetionet}),(c:ReferenceTherapeutic_reactome{identifier:line.id}) CREATE (d)-[: equal_to_reactome_drug]->(c) SET d.resource = split(line.resource, '|'), d.reactome = "yes";\n'''
