...
def create_cypher_file(directory, file_path, node_label, rela_name, direction1, direction2, start_label):
    if "Complex" in start_label:
        ...
        query = ''' MATCH (d:Protein{identifier:line.id_pharmebinet_Complex}),(c:%s{identifier:line.id_pharmebinet_node}) CREATE (d)%s[:%s{order:line.order, stoichiometry:line.stoichiometry, names:split(line.displayName,"|"), descriptions:split(line.description,"|"), pubMed_ids:split(line.pubMed_ids,"|"), books:split(line.books,"|"), resource: ['Reactome'], reactome: "yes", source:"Reactome", license:"CC BY 4.0", url:"https://reactome.org/content/detail/"+line.physicalEntity_id}]%s(c)'''
