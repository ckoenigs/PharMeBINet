...
def cypher_edge(file_name, label1, label2, properties, edge_name):
    ...
    query = query + ' source:"RNAInter", resource:["RNAInter"], licenses:["%s"],rnainter:true, url:"http://www.rnainter.org/showMore/?raid="+split(line.RNAInterID,"|")[0]}]->(p2)'
    query = query % (pharmebinetutils.dict_source_to_license['rnainter'])
