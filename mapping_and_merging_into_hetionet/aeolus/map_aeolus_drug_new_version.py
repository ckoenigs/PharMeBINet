...
def get_aeolus_connection_information_in_dict(number_of_compound_to_work_with):

    query ='''Match (c:Compound{aeolus:'yes'}) Where not exists(c.integrated)  Set c.integrated='yes' Return c.identifier Limit '''+ str(number_of_compound_to_work_with)
    results = g.run(query)
    for drugbank_id, in results:
        # drugbank_id = compound['identifier']
        query =''' Match (c:Compound)-[:equal_to_Aeolus_drug]-(r) Where c.identifier='%s'  Return r.drug_concept_id '''
        query = query %(drugbank_id)
        drug_concepts=g.run(query)
        for drug_concept_id, in drug_concepts:
...
# rela csv file
csv_file = open('cypher_map/rela.tsv', 'w')
fieldnames=['drugbank_id', 'cui', 'countA', 'prr_95_percent_upper_confidence_limit', 'prr', 'countB',
                 ...
                 'how_often_appears', 'resource', 'ror_min', 'ror_max', 'prr_min', 'prr_max']
...
def integrate_connection_from_aeolus_in_hetionet():
    ...
    for (drugbank_id, cui), information_lists in dict_connection_information.items():
        ...
        #         resource.append('AEOLUS')
        #     resource_2 = '","'.join(resource)
        #     resource='|'.join(resource)
        #     how_often_appears = str(int(first_connection['how_often_appears']) + 1) if first_connection['how_often_appears'] != None else '1'

            # l.hetionet='yes',
            # query = '''Match (c:Compound{identifier:"%s"})-[l:CAUSES_CcSE]-(r:SideEffect{identifier:"%s"})
            # Set l.countA="%s", l.prr_95_percent_upper_confidence_limit="%s", l.prr="%s", l.countB="%s", l.prr_95_percent_lower_confidence_limit="%s", l.ror="%s", l.ror_95_percent_upper_confidence_limit="%s", l.ror_95_percent_lower_confidence_limit="%s", l.countC="%s", l.drug_outcome_pair_count="%s", l.countD="%s",  l.aeolus='yes', l.how_often_appears="%s", l.resource=["%s"], l.ror_min="%s", l.ror_max="%s", l.prr_min="%s", l.prr_max="%s"; \n'''
            # query = query % (drugbank_id, cui, countA, prr_95_percent_upper_confidence_limit, prr, countB,
            #                  prr_95_percent_lower_confidence_limit, ror, ror_95_percent_upper_confidence_limit,
            #                  ror_95_percent_lower_confidence_limit, countC, drug_outcome_pair_count, countD, how_often_appears,
            #                  resource_2, ror_min, ror_max, prr_min, prr_max)
            # number_of_updated_connection += 1
        # only for this integration
        how_often_appears=str(1)
        resource=''

        values_list=[drugbank_id, cui, countA, prr_95_percent_upper_confidence_limit, prr, countB,
                         prr_95_percent_lower_confidence_limit, ror, ror_95_percent_upper_confidence_limit,
                         ror_95_percent_lower_confidence_limit, countC, drug_outcome_pair_count, countD,
                         how_often_appears, resource, ror_min, ror_max, prr_min, prr_max]
        information_dict=dict(zip(fieldnames,values_list))
...
def main():
    ...
    for x in range(0, running_times):
        ...
        get_aeolus_connection_information_in_dict(number_of_compounds_at_once)
