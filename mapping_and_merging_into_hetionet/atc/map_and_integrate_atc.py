...
def write_files(path_of_directory):
    ...
    query = '''Match (n:atc{identifier:line.id}), (v:Compound{identifier:line.compound_id}) Create (v)-[:equal_to_atc]->(n)'''
    query = pharmebinetutils.get_query_import(path_of_directory,
                                              f'mapping_and_merging_into_hetionet/atc/{file_name_mapped}',
                                              query)
    cypher_file.write(query)

    query = '''Match (n:atc{identifier:line.id}), (v:PharmacologicClass{identifier:line.pc_id}) Set v.atc_codes=[line.id], v.resource=split(line.resource,"|"), v.drugbank="yes" Create (v)-[:equal_to_atc]->(n)'''
    query = pharmebinetutils.get_query_import(path_of_directory,
                                              f'mapping_and_merging_into_hetionet/atc/{file_name_mapped_pc}',
                                              query)
    cypher_file.write(query)

    query = '''Match (n:atc{identifier:line.id}) Create (v:PharmacologicClass{identifier:line.id, drugbank:'yes', resource:['DrugBank'], source:'ATC from DrugBankT', url:'http://identifiers.org/atc:'+line.id, name:n.name, license:"Creative Common's Attribution-NonCommercial 4.0 International", class_type:["ATC code"], atc_codes:[line.id]}) Create (v)-[:equal_to_atc]->(n)'''
    query = pharmebinetutils.get_query_import(path_of_directory,
