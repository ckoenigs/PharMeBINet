...
def load_variants_from_database_and_add_to_dict():
    ...
    '''
    query = "MATCH (n:Variant) WHERE any(x in n.xrefs WHERE x =~ 'dbSNP:.*') RETURN n.identifier, n.resource, n.xrefs, n.licenses"
    results = g.run(query)
...
def generate_files(path_of_directory):
    ...
    header = ['DisGeNet_snp_id', 'identifier', 'resource', 'mapping_method', 'licenses']
    ...
    query = f'Match (n:variant_DisGeNet{{snpId:line.DisGeNet_snp_id}}), (v:Variant{{identifier:line.identifier}}) Set v.disgenet=true, v.resource=split(line.resource,"|"), v.licenses=split(line.licenses,"|") Create (v)-[:equal_to_DisGeNet_variant{{mapped_with:line.mapping_method}}]->(n)'
    ...
    query = f' Match (n:variant_DisGeNet{{snpId:line.identifier}}) Create (p:Variant :GeneVariant{{identifier:n.snpId, resource:["DisGeNet"], xrefs:["dbSNP:"+n.snpId], licenses:["{license}"], disgenet:true, source:"dbSNP from DisGeNet" }}) Create (p)-[:equal_to_DisGeNet_variant{{mapped_with:"new"}}]->(n)'
    query = pharmebinetutils.get_query_import(path_of_directory,
                                              'not_mapped.tsv',
                                              query)
    cypher_file.write(query)
...
def load_all_DisGeNet_variants_and_finish_the_files(csv_mapping):
    ...
    for record in results:
        ...
        if identifier in dict_variant_id_to_resource:
            ...
            dict_variant_id_to_resource[identifier][1].add(license)
            csv_mapping.writerow(
                [identifier, identifier,
                 pharmebinetutils.resource_add_and_prepare(dict_variant_id_to_resource[identifier][0], "DisGeNet"),
                 'external_references', '|'.join(dict_variant_id_to_resource[identifier][1])])
        else:
