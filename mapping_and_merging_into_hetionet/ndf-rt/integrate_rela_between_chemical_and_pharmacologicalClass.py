...
def write_files(label, direction_1, direction_2, rela_name):
    ...
    query = f'''Match (c:%s{{identifier:line.chemical_id}}), (p:PharmacologicClass{{identifier:line.pharmacological_class_id}}) Merge (c)%s[r:%s]%s(p) On Create Set r.source=line.source, r.resource=['NDF-RT'], r.url='http://purl.bioontology.org/ontology/NDFRT/'+line.pharmacological_class_id , r.licenses=['{license}'], r.unbiased=false, r.ndf_rt=true On Match Set r.resource=r.resource+'NDF-RT' ,r.licenses=r.licenses+'{license}' , r.ndf_rt=true '''
