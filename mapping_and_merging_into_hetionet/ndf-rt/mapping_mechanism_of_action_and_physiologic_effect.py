...
def write_files(path_of_directory, addition_name, label):
    ...
    query = '''Using Periodic Commit 10000 Load CSV  WITH HEADERS From "file:%smaster_database_change/mapping_and_merging_into_hetionet/%s" As line FIELDTERMINATOR '\\t' 
                Match (n:%s{code:line.code}) Create (v:PharmacologicClass{identifier:line.id, ndf_rt:'yes', xrefs:split(line.xrefs,'|'), synonyms:split(line.synonyms,'|'), resource:['NDF-RT'], source:'NDF-RT', url:'http://purl.bioontology.org/ontology/NDFRT/'+line.id, name:n.name, license:'', class_type:"%s"} Create (v)-[:equal_to_%s_ndf_rt{how_mapped:'new'}]->(n);'''
    query = query % (path_of_directory, file_name_mapped, label, addition_name,addition_name.replace('_',' ').title())
    cypher_file.write(query)
