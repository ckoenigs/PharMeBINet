...
def integration_of_ndf_rt_drugs_into_hetionet():
    ...
    query='''Match (b:Chemical)--(:NDFRT_DRUG_KIND)-[:effect_may_be_inhibited_by]->(:NDFRT_DRUG_KIND)--(c:Chemical) Merge (b)-[r:INTERACTS_CiC]->(c)  On Match Set r.resource=r.resource+'NDF-RT' On Create set r.resource=['NDF-RT'], r.source='NDF-RT', r.ndf_rt='yes, r.license='ndf-rt license'; '''
    cypher_file.write(query)
    cypher_file.close()
