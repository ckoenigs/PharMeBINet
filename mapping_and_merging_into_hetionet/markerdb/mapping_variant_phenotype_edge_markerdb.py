...
def load_edges_from_database_and_add_to_dict():
    ...
    '''
    print("query_started--------")
    query = "MATCH (n:variant)-[r]-(p:Phenotype) RETURN n.identifier,r.resource,p.identifier"
    results = g.run(query)
    print("query_ended----------")
    ...
    for record in results:
        [variant_id, resource, phenotype_id] = record.values()
        count += 1
        if count % 50000 == 0:
            print(f"process: {count}")
            print(datetime.datetime.now())
        if (variant_id, phenotype_id) in dict_pairs_to_info and dict_pairs_to_info[(variant_id, phenotype_id)] != resource:
            print('------ohje------')
            print(variant_id, phenotype_id)
            print(resource)
            print(dict_pairs_to_info[(variant_id, phenotype_id)])
        dict_pairs_to_info[(variant_id, phenotype_id)] = resource
...
def get_MarkerDB_information():
    '''
    Load all MarkerDB variant-variant-phenotype and save to tsv
    '''
    ...
    for record in results:
        ...
        if "Disease" in labels and (variant_id, phenotype_id) not in dict_all_mappings:
            ...
            print("Not mapped:", variant_id, phenotype_id, dict_all_mappings[(variant_id, phenotype_id)])
    file_disease.close()
    file_symptom.close()
...
def main():
    ...
    print(datetime.datetime.now())
    print('gather all information of the variants/phenotypes')
