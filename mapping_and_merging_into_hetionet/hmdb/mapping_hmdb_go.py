...
def load_hmdb_data_in_an_map_to_database(label, dict_pharmebinet, dict_alternative_ids_pharmebinet, csv_writer):
    ...
    for record in results:
        ...
        if go_id in dict_pharmebinet:
            dict_pharmebinet[go_id][1].add(pharmebinetutils.dict_source_to_license['hmdb'])
            csv_writer.writerow([go_id, go_id, 'identifier',
                                 pharmebinetutils.resource_add_and_prepare(dict_pharmebinet[go_id][0], 'HMDB'),
            ...
            for real_go_id in dict_alternative_ids_pharmebinet[go_id]:
                dict_pharmebinet[real_go_id][1].add(pharmebinetutils.dict_source_to_license['hmdb'])
                csv_writer.writerow([go_id, real_go_id, 'alternative_id',
                                     pharmebinetutils.resource_add_and_prepare(dict_pharmebinet[real_go_id][0], 'HMDB'),
...
def generate_files(label, label_hmdb):
    ...
    writer.writerow(['GOIDHMDB', 'GOIDpharmebinet', 'how_mapped', 'resource', 'licenses'])

    query = ''' Match (c:%s{ identifier:line.GOIDpharmebinet}), (n:%s{identifier:line.GOIDHMDB}) SET  c.hmdb=true, c.resource=split(line.resource,"|"), c.licenses=split(line.licenses,"|") Create (c)-[:equal_to_hmdb_go{how_mapped:line.how_mapped}]->(n)'''
