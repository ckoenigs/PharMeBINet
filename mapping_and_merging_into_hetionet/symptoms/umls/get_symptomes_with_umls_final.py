...
class symptomFinderThread(threading.Thread):
    def __init__(self, threadID, name, list_cui_with_synonyms):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = name
    ...
    def run(self):
        ...
        # Get lock to synchronize threads
        threadLock2.acquire()
        find_symptoms(self.list_cui_with_synonyms)
        #      print "Ending " + self.name
        #      print_time(self.name, self.counter, 3)
        # Free lock to release next thread
        threadLock2.release()
...
# dictionary of all diseases in hetionet with cui as key and doid as value
'''
cui1: doid
'''
...
# dictionary with all doids, which has no cuids, but xrefs
dict_doid_to_xrefs = {}

# list of doids whcih has no alternatic identifier
list_doid_without_refs = []

'''
load from hetionet all disease in and make a dictionary with the doid and umls cui
...
def load_in_hetionet_disease_in_dictionary():
    query = '''MATCH (n:Disease)  RETURN n.identifier,n.umls_cuis , n.xrefs Limit 500'''
    results = g.run(query)
    counter = 0
    count_cuis_double = 0
    for doid, umls_cuis, xrefs, in results:
        #        print(doid)
        #        print(umls_cuis)
        if len(umls_cuis) > 0 and not umls_cuis[0] == '':
            ...
            if not cui in dict_of_all_diseases:
                dict_of_all_diseases[cui] = [doid]
            else:
                count_cuis_double += 1
                dict_of_all_diseases[cui].append(doid)
        else:
            #            print('ne')
            #            print(doid)
            #            print(xrefs)
            if xrefs[0] != '':
                dict_doid_to_xrefs[doid] = xrefs
            else:
                list_doid_without_refs.append(doid)

    print('how often a cuis is found:' + str(counter))
    print('how often a cui appears double:' + str(count_cuis_double))
    print('length of dict with cui and list of doids:' + str(len(dict_of_all_diseases)))
    print('length of dict wit doid and xref list:' + str(len(dict_doid_to_xrefs)))
    print('length of disease ontology without a ref:' + str(len(list_doid_without_refs)))

    # get umls cui with use of other external ids
    i = 0
    list_doi = dict_doid_to_xrefs.keys()
    print(list_doi[0:5])
    j = 0
    for doid, xrefs in dict_doid_to_xrefs.items():
    ...
    print('length of dict wit doid and xref list:' + str(len(dict_doid_to_xrefs)))
    print('length of disease ontology without a ref:' + str(len(list_doid_without_refs)))
    print('length of dict with cui and list of doids after use of xref:' + str(len(dict_of_all_diseases)))
...
it generate for every disease a csv file with all synonyms,with form cui1;name1;cui2;name2;rel;rela;doid \n   
...
def find_synonyms(key):
    ...
    f.write('cui1;name1;cui2;name2;rel;rela;doid \n')
    # key='C0024117'
    ...
    for (cui1, rel, cui2, rela) in cur:
        ...
        # get the doid of the disease
        if cui1 in dict_of_all_diseases.keys():
            doid = dict_of_all_diseases[cui1]

        else:
            doid = dict_of_all_diseases[cui2]
        doid = '|'.join(doid)
        ...
        if is_synonym and synonym_cui not in synonym:
            ...
            if rela is None:
                text = cui1 + ';' + name1 + ';' + cui2 + ';' + name2 + ';' + rel + ';;' + doid + ' \n'
            else:
                text = cui1 + ';' + name1 + ';' + cui2 + ';' + name2 + ';' + rel + ';' + rela + ';' + doid + ' \n'
            f.write(text)
...
def integrate_symptoms_into_hetionet():
    ...
    counter_already_in_hetionet_symptioms = 0
    for symptom_cui, name in dict_symptoms_cuis.items():
        ...
        if first_entry == None:
            ...
            counter_already_in_hetionet_symptioms += 1
        g.run(query)

    query = '''MATCH (s:Symptom) Where not exists(s.umls) Set s.umls='no' '''
    g.run(query)
    print('number of new symptoms:' + str(counter_new_symptoms))
    print('number of symptoms which are already in hetionet:' + str(counter_already_in_hetionet_symptioms))
...
def generate_cypher_file_for_relationships():
    ...
    for cui, symptoms in dict_disease_symptomes.items():
        dois_ids = dict_of_all_diseases[cui]
        for doid in dois_ids:
            for symptom in symptoms:
                ...
                if not symptom in dict_umls_cui_to_mesh_or_umls_cui:
                    ...
                    if first_entry == None:
                        ...
                        query = query % (doid, mesh_or_cui, string_resource)
...
'''
#go throug the filtered file and save all cuis in the list 
...
def main():
    ...
    print('how many diseases with symptoms:' + str(len(list_synonyms)))

    for synonyms in list_synonyms:
        # create thread
        thread = symptomFinderThread(thread_id, 'thread_' + str(thread_id), synonyms)
        # start thread
        thread.start()
        # add to list
