...
# dictionary with umls cui as key and doid as value
dict_umls_cui_to_doid = {}

# dictionary with omim id as key and doid as value
dict_omim_to_doid = {}

'''
load all disease with and remember all name, synonym, umls cui and omim id
'''
...
def get_all_disease_information_from_hetionet():
    ...
    print('number of name to doid:' + str(len(dict_names_to_doids)))
...
# ditionary with pair(doiD, HPO id) and qualifier, db_reference, efidence code, frequcy modifier
dict_disease_symptom_hpo_pair_to_information = {}

# dictioary with HPO id as key and value is name and umls cui
dict_hpo_ID_to_information = {}

# list of disease ids which are not mapped to doid
list_not_mapped_disease_ids_to_doid = []

# dictionary of already mapped disease to doid
...
def gather_all_disease_symptom_information_from_HPO():
    ...
    for line in f:
        ...
        if db_disease != 'OMIM':
            ...
            if db_disease_name in dict_names_to_doids:
                doid = dict_names_to_doids[db_disease_name]
                #                print('decipher name')
                dict_disease_symptom_hpo_pair_to_information[(doid, hpo_id)] = [qualifier, reference_id, evidence_code,
                                                                                frequenc_modi]
                    ...
                    if len(names) > 1:
                        for name in names:
                            ...
                            for more_name in more_names:
                                if more_name in dict_names_to_doids:
                                    doid = dict_names_to_doids[more_name]
                                    #                print('decipher name')
                                    dict_disease_symptom_hpo_pair_to_information[(doid, hpo_id)] = [qualifier,
                                                                                                    reference_id,
                                                                                                    evidence_code,
...
'''
group the terms to gether and get the information from the different terms
and integrate the symptoms with umls cui into Hetionet
'''
...
def generate_cypher_file_for_connection():
    ...
    for (doid, hpo_id), information in dict_disease_symptom_hpo_pair_to_information.items():
        ...
        if first_entry == None:
            ...
            query = query % (doid, symptom_id, reference_id, qualifier, evidence_code, frequenc_modi, url)
            ...
            query = query % (
            doid, symptom_id, reference_id, qualifier, evidence_code, frequenc_modi, string_resource, url)
