...
def load_all_disease_in_dictionary():
    ...
    results = g.run(query)
    for identifier, name, xrefs, resource, licenses, in results:
        ...
        dict_diseases_in_resource[identifier] = [resource, set(licenses)]
...
csv_writer_included.writerow(['disease_id', 'mondo_id', 'resource', 'how_mapped', 'licenses'])
...
def generate_cypher_file():
    ...
    query_set = ' Match (n:Disease{identifier:line.mondo_id}), (b:DISEASES_Disease{id:line.disease_id}) Set  n.diseases=TRUE, n.resource=split(line.resource,"|"), n.licenses=split(line.licenses,"|") Create (n)-[:equal_to {how_mapped:line.how_mapped}]->(b)'
    print(query_set)
    ...
    for identifier, name, in g.run(query):
        ...
        if name and identifier != 'DOID:0050156':
            ...
            if name in dict_name_to_mondo_ids:
                ...
                for mondo_id in dict_name_to_mondo_ids[name]:
                    dict_diseases_in_resource[mondo_id][1].add(license)
                    csv_writer_included.writerow([identifier, mondo_id, pharmebinetutils.resource_add_and_prepare(
                        dict_diseases_in_resource[mondo_id][0], 'DISEASES'), 'name',
                                                  '|'.join(dict_diseases_in_resource[mondo_id][1])])
        ...
        if identifier in dict_doid_to_mondo_ids:
            ...
            for mondo_id in dict_doid_to_mondo_ids[identifier]:
                dict_diseases_in_resource[mondo_id][1].add(license)
                csv_writer_included.writerow([identifier, mondo_id, pharmebinetutils.resource_add_and_prepare(
                    dict_diseases_in_resource[mondo_id][0], 'DISEASES'), 'doid',
                                              '|'.join(dict_diseases_in_resource[mondo_id][1])])
