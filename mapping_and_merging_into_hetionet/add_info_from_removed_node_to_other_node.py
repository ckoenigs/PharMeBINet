...
def merge_resource_to_node(delete_node, label, merged_node):
    ...
    combined_resource=set(resources_list).union(dict_merge_node['resource']) if 'resource' in dict_merge_node else set()

    print('delete node:' + delete_node)
    print('merged node:' + merged_node)

    query_start= '''Match (s:%s{identifier:"%s"}) Set s.resource=["%s"], '''
...
def get_rela_info_and_add_cypher_queries(identifier, label, merged_node_id):
    ...
    for rela_type,  node_labels,  in results:
