...
def generate_cypher(header, file_name):
    ...
    query = '''Using Periodic Commit 10000 Load CSV  WITH HEADERS From "file:''' + path_of_directory + '''mapping_and_merging_into_hetionet/drugbank/compound_interaction/%s" As line  Fieldterminator '\\t' Match (c1:Compound{identifier:line.id1}), (c2:Compound{identifier:line.id2}) Create (c1)-[:RESEMBLES_CrC{source:"Open Babel and rdKit", unbiased:false, url:"https://pharmebi.net/#/compounds/"+line.id1, resource:['OpenBabel','rdKit'], open_babel_and_rdkit:'yes', license:'%s', '''
    for head in header:
