...
for i, drug in enumerate(root):
    ...
    row['amino_acid_sequence'] = drug.findtext(ns + "amino-acid-sequence").replace('\t', '').replace('\n', '').replace(
        '\r', '') if drug.findtext(ns + "amino-acid-sequence") != None else ''
    row['sequence'] = drug.findtext("{ns}sequences/{ns}sequence".format(ns=ns)).replace('\t', '').replace('\n',
                                                                                                          '').replace(
        '\r', '') if drug.findtext("{ns}sequences/{ns}sequence".format(ns=ns)) != None else ''
...
columns = ['drugbank_id', 'name', 'type', 'groups', 'atc_codes', 'categories', 'inchikey', 'inchi', 'inchikeys',
           'synonyms', 'unii', 'uniis', 'external_identifiers', 'extra_names', 'brands', 'molecular_forula',
           'molecular_formular_experimental', 'gene_sequence', 'amino_acid_sequence', 'sequence', 'description']
