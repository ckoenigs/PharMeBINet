...
map_file_name = "mapped_CAERSReport_reaction_SideEffect.tsv"
#######################################################################
# f = open("FDA_mappings/" + cypher_file_name, 'w')
# f.close()
#######################################################################
print(datetime.datetime.utcnow())
print("Fetching data for CAERSReport_reaction_openFDA ...")
# FDA Daten abrufen und anschließend als Dictionary speichern.
query = "MATCH (n:CAERSReport_reaction_openFDA) RETURN n.id, toLower(n.reactions);"
a = list(g.run(query))
print(datetime.datetime.utcnow())
...
for entry in a:
    id_ = entry["n.id"]
    attr_ = entry["toLower(n.reactions)"]
...
#######################################################################
FDA_name = "CAERSReport_reaction_openFDA"
CAT_name = "SideEffect"
FDA_attr = [["id", "name"]]
CAT_attr = [["identifier", "name"]]
_map = ["name"]
nonmap_file_name = ["nonmapped_CAERSReport_reaction_reaction.tsv"]
...
cur.execute(query)
#######################################################################
map_others(cur, CAT, "nonmapped_CAERSReport_reaction_reaction",
           "mapped_CAERSReport_reaction_SideEffect")
#######################################################################
