...
bp_writer.writerow(
    ['ChemicalID', 'GOID', 'targetTotalQty', 'backgroundTotalQty', 'backgroundMatchQty', 'correctedPValue', 'pValue',
     'targetMatchQty', 'chemicalID'])
...
def add_information_into_te_different_tsv_files(chemical_id, go_id, information, csvfile):
    ...
    csvfile.writerow(
        [chemical_id, go_id, targetTotalQtys, backgroundTotalQtys, backgroundMatchQtys, correctedPValues, pValues,
         targetMatchQtys, chemicaL_mesh_id])
...
def take_all_relationships_of_gene_pathway():
    ...
    query = ''' Match (n:Chemical{identifier:line.ChemicalID}), (b:BiologicalProcess{identifier:line.GOID}) Merge (n)-[r:ASSOCIATES_CHaBP]->(b) On Create Set  r.targetTotalQty=split(line.targetTotalQty, '|'), r.backgroundTotalQty=split(line.backgroundTotalQty,'|'), r.backgroundMatchQty=split(line.backgroundMatchQty,'|'), r.correctedPValue=split(line.correctedPValue,'|'), r.pValue=split(line.pValue,'|'), r.targetMatchQty=split(line.targetMatchQty,'|'), r.ctd='yes', r.url_ctd="http://ctdbase.org/detail.go?type=chem&acc="+line.ChemicalID, r.source="CTD", r.license="© 2002–2012 MDI Biological Laboratory. © 2012–2024 NC State University. All rights reserved.", r.unbiased=false On Match SET r.targetTotalQty=split(line.targetTotalQty, '|'), r.backgroundTotalQty=split(line.backgroundTotalQty,'|'), r.backgroundMatchQty=split(line.backgroundMatchQty,'|'), r.correctedPValue=split(line.correctedPValue,'|'), r.pValue=split(line.pValue,'|'), r.targetMatchQty=split(line.targetMatchQty,'|'), r.ctd='yes', r.url_ctd="http://ctdbase.org/detail.go?type=chem&acc="+line.ChemicalID '''
    query = pharmebinetutils.get_query_import(path_of_directory,
                                              f'mapping_and_merging_into_hetionet/ctd/chemical_go/bp.tsv',
                                              query)
    cypherfile.write(query)
    cypherfile.close()
    cypherfile = open('chemical_go/cypher_mf.cypher', 'w')
    query = ''' Match (n:Chemical{identifier:line.ChemicalID}), (b:MolecularFunction{identifier:line.GOID}) Merge (n)-[r:ASSOCIATES_CHaMF]->(b) On Create Set  r.targetTotalQty=split(line.targetTotalQty, '|'), r.backgroundTotalQty=split(line.backgroundTotalQty,'|'), r.backgroundMatchQty=split(line.backgroundMatchQty,'|'), r.correctedPValue=split(line.correctedPValue,'|'), r.pValue=split(line.pValue,'|'), r.targetMatchQty=split(line.targetMatchQty,'|'), r.ctd='yes', r.url_ctd="http://ctdbase.org/detail.go?type=chem&acc="+line.ChemicalID, r.source="CTD", r.license="© 2002–2012 MDI Biological Laboratory. © 2012–2024 NC State University. All rights reserved.", r.unbiased=false On Match SET r.targetTotalQty=split(line.targetTotalQty, '|'), r.backgroundTotalQty=split(line.backgroundTotalQty,'|'), r.backgroundMatchQty=split(line.backgroundMatchQty,'|'), r.correctedPValue=split(line.correctedPValue,'|'), r.pValue=split(line.pValue,'|'), r.targetMatchQty=split(line.targetMatchQty,'|'), r.ctd='yes', r.url_ctd="http://ctdbase.org/detail.go?type=chem&acc="+line.ChemicalID '''
    query = pharmebinetutils.get_query_import(path_of_directory,
                                              f'mapping_and_merging_into_hetionet/ctd/chemical_go/mf.tsv',
                                              query)
    cypherfile.write(query)
    cypherfile.close()
    cypherfile = open('chemical_go/cypher_cc.cypher', 'w')
    query = ''' Match (n:Chemical{identifier:line.ChemicalID}), (b:CellularComponent{identifier:line.GOID}) Merge (n)-[r:ASSOCIATES_CHaCC]->(b) On Create Set  r.targetTotalQty=split(line.targetTotalQty, '|'), r.backgroundTotalQty=split(line.backgroundTotalQty,'|'), r.backgroundMatchQty=split(line.backgroundMatchQty,'|'), r.correctedPValue=split(line.correctedPValue,'|'), r.pValue=split(line.pValue,'|'), r.targetMatchQty=split(line.targetMatchQty,'|'), r.ctd='yes', r.url_ctd="http://ctdbase.org/detail.go?type=chem&acc="+line.ChemicalID, r.source="CTD", r.license="© 2002–2012 MDI Biological Laboratory. © 2012–2024 NC State University. All rights reserved.", r.unbiased=false On Match SET r.targetTotalQty=split(line.targetTotalQty, '|'), r.backgroundTotalQty=split(line.backgroundTotalQty,'|'), r.backgroundMatchQty=split(line.backgroundMatchQty,'|'), r.correctedPValue=split(line.correctedPValue,'|'), r.pValue=split(line.pValue,'|'), r.targetMatchQty=split(line.targetMatchQty,'|'), r.ctd='yes', r.url_ctd="http://ctdbase.org/detail.go?type=chem&acc="+line.ChemicalID '''
    query = pharmebinetutils.get_query_import(path_of_directory,
                                              f'mapping_and_merging_into_hetionet/ctd/chemical_go/cc.tsv',
                                              query)
    cypherfile.write(query)
    cypherfile.close()
    ...
    query = '''Match (n:Chemical) Where (n)-[:equal_chemical_CTD]-() or (n)-[:equal_to_CTD_chemical]-() Return count(n) as v'''
    results = g.run(query)
    number_of_disease = int(results.single()['v'])
    ...
    while counter_of_used_chemical < number_of_disease:
        ...
        for record in results:
            [chemical_id, drugbank_ids, rela, go_id, ontology] = record.values()
            counter += 1
            rela = dict(rela)
            ...
            if chemical_id in list_durgbank_mesh:
                if len(drugbank_ids) > 0:
                    for drugbank_id in drugbank_ids:
                        if (drugbank_id, go_id) in dict_chemical_go:
                            ...
                            dict_chemical_go[(drugbank_id, go_id)] = [set([correctedPValue]), set([targetTotalQty]),
                                                                      set([backgroundMatchQty]),
                                                                      set([targetMatchQty]), set([pValue]),
                                                                      set([backgroundTotalQty]), set([ontology]),
                                                                      set([chemical_id])]
                ...
                if (chemical_id, go_id) in dict_chemical_go:
                    ...
                    dict_chemical_go[(chemical_id, go_id)] = [set([correctedPValue]), set([targetTotalQty]),
                                                              set([backgroundMatchQty]),
                                                              set([targetMatchQty]), set([pValue]),
                                                              set([backgroundTotalQty]), set([ontology]),
                                                              set([chemical_id])]
