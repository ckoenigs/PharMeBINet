...
java -jar ../$import_tool.jar -i sources/PTMD/intermediate.graphml.gz  -e bolt://localhost:$neo4j_bolt --username neo4j --password $password --label-prefix PTMD_ --indices "PTMD_Protein.uniprot_accession;PTMD_Disease.name" > output/import_tool_output.txt
