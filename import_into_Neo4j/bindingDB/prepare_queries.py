...
edges_id = [['MONO_STRUCT_NAMES', 'COBWEB_BDB', 'MONOMER_ID', 'MONOMERID'], ['MONO_STRUCT_NAMES', 'COMPLEX_COMPONENT', 'MONOMERID', 'MONOMERID'],
            ['MONO_STRUCT_NAMES', 'ENZYME_REACTANT_SET', 'ENZYME_MONOMERID', 'MONOMERID'], ['POLYMER_AND_NAMES', 'ENZYME_REACTANT_SET', 'ENZYME_POLYMERID', 'POLYMERID'],
            ['POLYMER_AND_NAMES', 'COMPLEX_COMPONENT', 'POLYMERID', 'POLYMERID'], ['ENZYME_REACTANT_SET', 'COBWEB_BDB', 'REACTANT_SET_ID', 'REACTANT_SET_ID'],
            ['ENTRY', 'ENZYME_REACTANT_SET', 'ENTRYID', 'ENTRYID'], ['COMPLEX_AND_NAMES', 'ENZYME_REACTANT_SET', 'ENZYME_COMPLEXID', 'COMPLEXID'],
            ['ENZYME_REACTANT_SET', 'PDB_BDB', 'REACTANT_SET_ID_STR', 'REACTANT_SET_ID'], ['ENTRY', 'ASSAY', 'ENTRYID', 'ENTRYID'], ['ASSAY', 'KI_RESULT', 'ASSAYID', 'ASSAYID'],
            ['ENTRY', 'KI_RESULT', 'ENTRYID', 'ENTRYID'], ['DATA_FIT_METH', 'KI_RESULT', 'DATA_FIT_METH_ID', 'DATA_FIT_METH_ID'], ['INSTRUMENT', 'KI_RESULT', 'INSTRUMENTID', 'INSTRUMENTID'],
            ['DATA_FIT_METH', 'ITC_RESULT_A_B_AB', 'DATA_FIT_METH_ID', 'DATA_FIT_METH_ID'], ['INSTRUMENT', 'ITC_RESULT_A_B_AB', 'INSTRUMENTID', 'INSTRUMENTID'],
            ['COMPLEX_AND_NAMES', 'COMPLEX_COMPONENT', 'COMPLEXID', 'COMPLEXID']]


def create_node_queries(tab):
    with open('create_nodes.cypher', 'w') as file:
        ...
        for table in tab:
            if table not in ['COMPLEX_AND_NAMES', 'POLYMER_AND_NAMES', 'MONO_STRUCT_NAMES']:
                ...
                query = query[:-1]
                query += "})"
                query = pharmebinetutils.get_query_import("/home/hela/Desktop/PharMeBINet/import_into_Neo4j/bindingDB",
                                                          "tsv_from_mysql/" + table + ".tsv", query)
                file.write(query)
            else:
                query = "create (:" + table + "{"
                file_name = "tsv_from_mysql/" + table + ".tsv"
...
def create_edge_queries(simple_edges, edges):
    with open('create_edges.cypher', 'w') as file:
        for edge in simple_edges:
            file_path = "/home/hela/Desktop/PharMeBINet/import_into_Neo4j/bindingDB/idx_tsv"
            if edge[0] == 'ASSAY' and edge[1] == 'KI_RESULT':
                ...
                query = pharmebinetutils.get_query_import(file_path, 'ASSAY-KI_RESULT.tsv', query)
                file.write(query)
            else:
                ...
                if edge[0] == 'ENZYME_REACTANT_SET' and edge[1] == 'PDB_BDB':
                    query = "MATCH (n1:ENZYME_REACTANT_SET), (n2:PDB_BDB{reactant_set_id_str:line.REACTANT_SET_ID_STR}) " \
                            ...
                            "\nCreate (n1) -[:RELATIONSHIP] -> (n2)"
                    query = pharmebinetutils.get_query_import(file_path, file_name, query)
                    file.write(query)
                else:
        ...
        for edge in edges:
            ...
            query = query + "]->(n2)"
            query = pharmebinetutils.get_query_import("/home/hela/Desktop/PharMeBINet/import_into_Neo4j/bindingDB",
                                                      "tsv_from_mysql/" + edge[1] + ".tsv", query)
            file.write(query)
...
def create_index_queries(edges, edges_2):
    with open('create_index.cypher', 'w') as file:
        queries_list = []
        for edge in edges:
            query = "CREATE INDEX index"+edge[1]+"_" + edge[2].lower() + " FOR (node:" + edge[1] +") ON (node." + edge[2].lower() + ");\n"
            queries_list.append(query)
            query = "CREATE INDEX index"+edge[0]+"_" + edge[3].lower() + " FOR (node:" + edge[0] +") ON (node." + edge[3].lower() + ");\n"
            queries_list.append(query)
...
def create_idx_tsv(edges_id):
    ...
    for chunk in pd.read_csv("tsv_from_mysql/KI_RESULT.tsv", sep='\t', usecols=columns_to_keep, chunksize=batch_size, na_values=[np.nan, '']):
