...
def get_geneInfo(gene_ids):
    ...
    file3 = file3.drop(columns=['geneSymbol', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType'])
    file1 = file1.drop(columns=['geneSymbol', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType'])
...
def get_variantInfo(snp_ids):
    ...
    file1 = file1.drop(
        columns=['chromosome', 'position', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType'])
    disease_df = file2[
        ['diseaseId', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType']].drop_duplicates(
        keep='first')  # needed for later use
    file2 = file2.drop(
    ...
    pmidsFrame = file2.groupby(['snpId', 'diseaseId'], as_index=False).agg(
        {'pmid': set})  # take set to not aggregate duplicates
    # calculate the number of entries for each list under pmid-column
    pmidsFrame['check_num'] = pmidsFrame.apply(lambda df: len(df['pmid']), axis=1)
    pmidsFrame['pmid'] = pmidsFrame['pmid'].apply(
        lambda x: '|'.join(list(x)))  # convert column of sets to column of lists
    # Merge gene-disease table with extra pmid info
...
def main():
    ...
    edge_dict = {'variant-disease.tsv': 'associates', 'gene-protein.tsv': 'interacts', 'gene-disease.tsv': 'interacts', 'variant-gene.tsv': 'associates'}
