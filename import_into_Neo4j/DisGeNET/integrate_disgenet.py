...
def cypher_node(cypher_file, filename, label, properties, unique_property):
    ...
    query_start = 'Using Periodic Commit 10000 Load CSV  WITH HEADERS From "file:' + os.path.join(home, destination+filename) + '" As line fieldterminator "\\t" '
...
def cypher_edge(cypher_file, filename, label, properties, id_list):
    """
    :param cypher_file: destination file to write the queries to
    :param filename: name of source file
    ...
    query_start = 'Using Periodic Commit 10000 Load CSV  WITH HEADERS From "file:' + os.path.join(home, destination+filename) + '" As line fieldterminator "\\t" '
    query = query_start + f'Match (p1:{label[0]}_DisGeNet{{{id_list[0]}:line.{id_list[0]}}}),(p2:{label[1]}_DisGeNet{{{id_list[1]}:line.{id_list[1]}}}) Create (p1)-[:interacts{{  '
...
def get_proteinInfo():
    ...
    file1.rename(columns={'GENEID':'geneId'}, inplace=True) # need to rename column "GENEID" to "geneId" for later coherence
...
def get_geneInfo(gene_ids):
    ...
    right = file1[['geneId', 'geneSymbol']].drop_duplicates(subset=['geneSymbol'])
    ...
    disease_df = file3[['diseaseId', 'diseaseName', 'diseaseType', 'diseaseClass','diseaseSemanticType']].drop_duplicates(keep='first')  # needed for later use
    ...
    pmidsFrame = file3.groupby(['geneId','diseaseId'], as_index=False).agg({'pmid': set})
    ...
    pmidsFrame['pmid'] = pmidsFrame['pmid'].apply(lambda x: '|'.join(list(x)))
...
def get_variantInfo(snp_ids):
    ...
    right = file1[['snpId', 'chromosome', 'position']].drop_duplicates(keep='first')
    ...
    file1 = file1.drop(columns=['chromosome', 'position', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType'])
    disease_df = file2[['diseaseId', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType']].drop_duplicates(keep='first')  # needed for later use
    file2 = file2.drop(columns=['chromosome', 'position', 'diseaseName', 'diseaseType', 'diseaseClass', 'diseaseSemanticType'])
    curated = ['GWASCAT', 'CLINVAR', 'GWASDB', 'UNIPROT', 'GWASCAT;GWASDB', 'CLINVAR;UNIPROT', 'CLINVAR;GWASCAT', 'CLINVAR;GWASDB', 'CLINVAR;GWASCAT;GWASDB', 'GWASCAT;GWASDB;UNIPROT', 'GWASCAT;UNIPROT']
    file2 = file2[file2.source.isin(curated)]
    # drop the rows that have NaN-entries in pmid
    file2 = file2.dropna(subset=['pmid'])
    file2 = file2.merge(file1[['snpId', 'diseaseId']], on=['snpId', 'diseaseId'], how='inner', copy=False, sort=True) #include column information of other file1
    # Convert dtypes of YearInitial, YearFinal, pmid to INT
    ...
    # aggregate the pmids for each gene + disease!!!
    pmidsFrame = file2.groupby(['snpId', 'diseaseId'], as_index=False).agg({'pmid': set}) # take set to not aggregate duplicates
    # calculate the number of entries for each list under pmid-column
    pmidsFrame['check_num'] = pmidsFrame.apply(lambda df: len(df['pmid']), axis=1)
    pmidsFrame['pmid'] = pmidsFrame['pmid'].apply(lambda x: '|'.join(list(x)))  # convert column of sets to column of lists
    # Merge gene-disease table with extra pmid info
...
def get_diseaseInfo(disease_ids_genetbl, disease_ids_vartbl, disease_df_genetbl, disease_df_vartbl):
    ...
    file1_grouped = file1.groupby(['diseaseId', 'diseaseName'], as_index=False).agg({'code': set, 'synonyms': set})  # take set to not aggregate duplicates
...
def main():
    ...
    output_files = [f for f in os.listdir(destination) if os.path.isfile(os.path.join(destination, f)) and f.endswith(".tsv")]
    ...
    id_dict = {'variant-disease.tsv': ['snpId', 'diseaseId'], 'gene-protein.tsv': ['geneId', 'UniProtKB'], 'gene-disease.tsv': ['geneId', 'diseaseId'], 'variant-gene.tsv': ['snpId', 'geneId']}
    for curr_file in output_edges:
        label = curr_file.split('.', 1)[0].split('-') # split the names into list with two node entries (make sure that the order of id_dict keys and order of the keys in list match)
        header = list(pd.read_csv(os.path.join(destination, curr_file), nrows=0, sep='\t').columns)
        id_list = id_dict[curr_file]
        #print(f'file: {curr_file} \nlabel: {label} \nproperties: {header} \nid_list: {id_list}\n')
        cypher_edge(cypher_file, curr_file, label, header, id_list)
