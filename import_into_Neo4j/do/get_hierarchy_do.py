...
def generate_files():
    ...
    query_start = ''' Match (a:DiseaseOntology)'''
    query_add_part = '''-[:is_a]->('''
    query_end_match = '''-[:is_a]->(b:DiseaseOntology{id:'DOID:4'}) Where '''
    query_where_level_element = '''not ()-[:is_a]->(a) Return a.id, a.name, '''
    query_where_level_class = ''' ()-[:is_a]->(a) Return a.id, a.name, '''

    count_level = 1
    letter_of_parent = 0
    while count_level <= 12:
...
def generate_tree():
    count_level = 1
    datatree = recursion_tree_generation("DOID:4", count_level) + "disease-DOID_4;"
    print(datatree)
