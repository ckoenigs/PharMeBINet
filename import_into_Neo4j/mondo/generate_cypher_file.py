...
def generate_cypher_file_with_apoc(save_path):
    ...
    query = '''call apoc.export.cypher.query("MATCH (n:disease)-[r]->(b:disease) Where n.`http://www.geneontology.org/formats/oboInOwl#id` contains 'MONDO' and b.`http://www.geneontology.org/formats/oboInOwl#id` contains 'MONDO' RETURN n,r,b","file:/home/cassandra/Dokumente/Project/master_database_change/import_into_Neo4j/mondo/mondo.cypher", {batchSize:10000}); '''
    ...
    query= '''call apoc.export.cypher.query("MATCH (n:disease) Where n.`http://www.geneontology.org/formats/oboInOwl#id`='MONDO:0013239' Return n","file:/home/cassandra/Dokumente/Project/master_database_change/import_into_Neo4j/mondo/single_node_without_connection.cypher", {batchSize:10}); '''
    g.run(query)
